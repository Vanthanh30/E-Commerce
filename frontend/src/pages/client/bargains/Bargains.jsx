...
const Bargains = () => {
  ...
  const groupedBargains = useMemo(() => ({
    ...
    completed: bargains.filter((item) => FINISHED_STATUSES.includes(item.sessionStatus || item.status)),
  }), [bargains]);
  ...
  const renderActionButton = (item) => {
    ...
    if (FINISHED_STATUSES.includes(sessionStatus)) {
      return (
        <button className="btn btn-outline" onClick={() => handleOpenChat(item)}>
          Xem lịch sử
        </button>
      );
    }
        ...
        <>
          {groupedBargains.negotiating.length > 0 && (
            <section style={{ marginBottom: "40px" }}>
              <h2 style={{ fontSize: "18px", fontWeight: "600", marginBottom: "16px", color: "#ea580c" }}>
          ...
          {groupedBargains.completed.length > 0 && (
            <section>
              <h2 style={{ fontSize: "18px", fontWeight: "600", marginBottom: "16px", color: "#6b7280" }}>
